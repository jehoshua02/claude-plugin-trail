...
`bash "$(ls -d ~/.claude/plugins/cache/jehoshua02/trail/*/scripts/new.sh | sort -V | tail -1)" "<topic-name>"`

If `$ARGUMENTS` is provided, run immediately:

`bash "$(ls -d ~/.claude/plugins/cache/jehoshua02/trail/*/scripts/new.sh | sort -V | tail -1)" "$ARGUMENTS"`
